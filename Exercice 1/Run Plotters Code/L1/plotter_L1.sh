...
for bench in "${BenchArray[@]}"; do
    python ~/advcomparch-2020-helpers/L1/graph_L1.py "false" "${bench}" ${OUT_PATH}/${bench}_L1*
    mv L1.png L1_${bench}.png
done

# Create the graphs for the second question
for bench in "${BenchArray[@]}"; do
    python ~/advcomparch-2020-helpers/L1/graph_L1.py "true" "${bench}, IPC Reduction" ${OUT_PATH}/${bench}_L1*
