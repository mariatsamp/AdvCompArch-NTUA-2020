...
for bench in "${BenchArray[@]}"; do
    python ~/advcomparch-2020-helpers/L2/graph_L2.py "false" ${bench} ${OUT_PATH}/${bench}_L2*
    mv L2.png L2_${bench}.png
done

# Create the graphs for the second question
for bench in "${BenchArray[@]}"; do
    python ~/advcomparch-2020-helpers/L2/graph_L2L.py "true" "${bench}, IPC Reduction" ${OUT_PATH}/${bench}_L2*
